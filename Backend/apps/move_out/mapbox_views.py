...
class MapboxGeocodeView(APIView):
    ...
    def get(self, request):
        ...
        if not postcode:
            return JsonResponse({"ok": False, "message": "Postcode query parameter is required."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if MAPBOX_TOKEN:
            try:
                geocode_url = f"https://api.mapbox.com/geocoding/v5/mapbox.places/{quote_plus(postcode)}.json"
                params = {
                    "country": "gb",
                ...
                if resp.ok:
                    ...
                    if feature:
                        ...
                        if isinstance(longitude, (int, float)) and isinstance(latitude, (int, float)):
                            result = {"longitude": longitude, "latitude": latitude, "placeName": place_name}
            except Exception:
                result = None
        ...
        if not result:
            try:
                resp = requests.get(f"https://api.postcodes.io/postcodes/{quote_plus(postcode)}", timeout=10)
                if resp.ok:
                    payload = resp.json()
                    ...
                    if isinstance(longitude, (int, float)) and isinstance(latitude, (int, float)):
                        ...
                        place_name = ", ".join([p for p in place_parts if p]) or res.get("postcode") or postcode
                        result = {"longitude": longitude, "latitude": latitude, "placeName": place_name}
            except Exception:
                result = None

        if not result:
            return JsonResponse({"ok": False, "message": "Could not find coordinates for this postcode."}, status=status.HTTP_404_NOT_FOUND)

        response = JsonResponse({"ok": True, "result": result}, status=status.HTTP_200_OK)
        response["Cache-Control"] = "public, s-maxage=86400, stale-while-revalidate=3600"
        return response
...
class MapboxStaticView(APIView):
    ...
    def get(self, request):
        ...
        def build_fallback_svg(has_marker: bool) -> str:
            marker_text = "Marker preview available" if has_marker else "UK overview fallback"
            return (
                ...
                "</g><circle cx=\"640\" cy=\"360\" r=\"14\" fill=\"#1d4ed8\" opacity=\"0.9\"/>"
                "<circle cx=\"640\" cy=\"360\" r=\"34\" fill=\"#1d4ed8\" opacity=\"0.2\"/>"
                f"<text x=\"640\" y=\"660\" text-anchor=\"middle\" font-family=\"Arial, sans-serif\" font-size=\"28\" fill=\"#334155\">Map unavailable right now - using fallback preview</text>"
                f"<text x=\"640\" y=\"695\" text-anchor=\"middle\" font-family=\"Arial, sans-serif\" font-size=\"20\" fill=\"#475569\">{marker_text}</text></svg>"
            )
        ...
        try:
            ...
            return JsonResponse({"ok": False, "message": "invalid lng/lat values"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        marker_overlay = f"pin-s+1d4ed8({center_lng:.5f},{center_lat:.5f})/" if has_marker else ""
        static_map_url = f"https://api.mapbox.com/styles/v1/mapbox/{style_id}/static/{marker_overlay}{center_lng:.5f},{center_lat:.5f},{zoom:.2f},0/1280x720@2x"
        params = {"access_token": MAPBOX_TOKEN, "logo": "false", "attribution": "false"}
        ...
        if not resp.ok:
            ...
            logger.warning("Mapbox static upstream failed: %s %s", resp.status_code, text_preview[:1000])
            svg = build_fallback_svg(has_marker)
            response = HttpResponse(svg, status=200, content_type="image/svg+xml")
            response["Cache-Control"] = "public, s-maxage=300"
            return response
        ...
        response = StreamingHttpResponse(resp.raw, status=resp.status_code, content_type=resp.headers.get("Content-Type", "image/png"))
        response["Cache-Control"] = "public, s-maxage=86400, stale-while-revalidate=3600"
        return response
